...
public class cnFieldParseInfo {
	...
	private cnType type;
	private int length;
    private String encoding = "UTF-8"; //报文内的中文编码
	...
	public cnValue<?> parse(byte[] buf, int pos) throws ParseException {
		if (type == cnType.NUMERIC || type == cnType.ALPHA) {
			...
			try {
                return new cnValue<String>(type, new String(buf, pos + 2, length, encoding));
            } catch (UnsupportedEncodingException e) {
                throw new ParseException(e.toString(), pos);
            }
		} else if (type == cnType.LLLVAR) {
			length = ((buf[pos] - 48) * 100) + ((buf[pos + 1] - 48) * 10) + (buf[pos + 2] - 48);
			try {
			return new cnValue<String>(type, new String(buf, pos + 3, length, encoding));
            } catch (UnsupportedEncodingException e) {
                throw new ParseException(e.toString(), pos);
            }
		...
		//源码丢失, 反编译救回的
		else if (this.type == cnType.DATE8) {
		      Calendar cal = Calendar.getInstance();
		      cal.set(10, 0);
		      cal.set(12, 0);
		      cal.set(13, 0);
		      cal.set(1, cal.get(1) - cal.get(1) % 100 + 
